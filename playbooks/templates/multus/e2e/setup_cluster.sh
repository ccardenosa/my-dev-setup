...
if [ "${running}" != 'true' ]; then
  ...
  while sleep 0.5; do
    echo "Pushing image..."
    $OCI_BIN push localhost:5000/multus:e2e --tls-verify=false
    [[ $($OCI_BIN search localhost:5000/multus --tls-verify=false) != "" ]] && break
  done
fi
reg_host="${reg_name}"
if [[ "$(podman network inspect ${kind_network} -f {{.Driver}})" == "bridge" ]]; then
    reg_host="$($OCI_BIN inspect -f '{{.NetworkSettings.IPAddress}}' "${reg_name}")"
fi
echo "Registry Host: ${reg_host}"

nohup bash setup_cluster_extra.sh &
